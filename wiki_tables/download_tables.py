...
class TableExtractor():
	...
	def load_wordlist(self,filename):
		...
		:return: список слов
		'''
		wordlist_path = os.path.abspath(self.file_path + '/wordlists/' + filename)
		...
		for word in wordlist:  # для каждого существительного в списке загружаем текст его страницы
			...
			for table in tables:
				...
				if len(strings) == 9 and len(strings[2]) == 3 \
						...
						or len(strings) == 8 and len(
							strings[2]) == 13:  # если таблица правильного формата, то продолжаем с ней работу,
					# иначе говорим, что таблица неверна.
			...
			if len(
					new_tables) == 2:  # если со страницы получено 2 таблицы, то это таблица склонения по падежам и числам и таблица притяжательных форм
				possessives = []
