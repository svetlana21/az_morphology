...
class LCS:
	...
	def lcs_as_lemmas(self, morphodict):
		...
		(технически это список во избежание совпадений парадигм при совпадении их новых лемм).
		:param morphodict: исходный словарь
		:return: список, где на месте леммы НОП
		'''
		print('Create a new dictionary with the longest common subsequences...')
		new_dict = []
	...
	def load_json(self, filename):
		...
		json_path = os.path.abspath(self.file_path + '/tables/' + filename)
	...
	def write_json(self, filename, dct):
		...
		json_path = os.path.abspath(self.file_path + '/tables_lcss/' + filename)
		with open(json_path, 'w', encoding='utf-8-sig') as f:
			json.dump(dct, f, ensure_ascii=False)

# Следующие две функции не используются
